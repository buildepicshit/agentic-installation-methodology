# Contributing to agentic-installation-methodology

This is the methodology repo for **agentic installation** — the
practice of taking a working product and decomposing it into an
exhaustive non-prescriptive deployment-grade SPEC bundle that
user-dispatched coding agents can install. It eats its own dog
food: contributions go through the same lifecycle the
methodology documents. Non-trivial changes require an IDEA →
SPEC pair before code lands.

## Trivial changes

Typo fixes, single-line doc clarifications, single-file
fixes ≤ 50 lines: use the Fastpath SPEC pattern (see
`templates/SPEC.fastpath.template.md`). Author a single
...
1. **Read `AGENTS.md`** — the entry doc covering posture and
   lifecycle.
