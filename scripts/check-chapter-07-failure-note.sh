#!/usr/bin/env bash
# check-chapter-07-failure-note.sh — T-08 helper.
#
# Verifies Phase 6 failure-mode note uses §6.5 three-
# element structure with explicit bold markers +
# judgment://agent-synthesis tag.
...
if [ ! -f "$CHAPTER" ]; then
    printf 'check-chapter-07-failure-note: FAIL — chapter file not found at %s\n' "$CHAPTER" >&2
    exit 1
fi
...
if [ -z "$section" ]; then
    printf 'check-chapter-07-failure-note: FAIL — could not extract failure-mode section\n' >&2
    exit 1
fi

section_heading="$(printf '%s\n' "$section" | head -n 1)"
if ! printf '%s' "$section_heading" | grep -iqE 'failure|what we|phase 6|ship'; then
    printf 'check-chapter-07-failure-note: FAIL — section heading %q does not contain a failure-concept anchor\n' "$section_heading" >&2
    exit 1
fi
...
if [ ${#errors[@]} -gt 0 ]; then
    printf 'check-chapter-07-failure-note: %d element(s) missing:\n' "${#errors[@]}" >&2
    for e in "${errors[@]}"; do
        printf '  - %s\n' "$e" >&2
    done
    exit 1
...
printf 'check-chapter-07-failure-note: PASS — three explicit bold markers + agent-synthesis tag on E3\n'
exit 0
