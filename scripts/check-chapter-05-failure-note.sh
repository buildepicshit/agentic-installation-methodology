#!/usr/bin/env bash
# check-chapter-05-failure-note.sh — T-06 helper.
#
# Verifies longread/05-phase-4-manifest-catalog-compose.md
# contains a Phase 4 failure-mode note in the §6.5 three-
# element structure as the chapter's penultimate section.
#
# Per T-04/T-05 lessons, each element requires an explicit
# bold marker AND E3 requires a judgment://agent-synthesis
# tag.
#
...
if [ ! -f "$CHAPTER" ]; then
    printf 'check-chapter-05-failure-note: FAIL — chapter file not found at %s\n' "$CHAPTER" >&2
    exit 1
fi
...
if [ -z "$section" ]; then
    printf 'check-chapter-05-failure-note: FAIL — could not extract failure-mode section\n' >&2
    exit 1
fi

section_heading="$(printf '%s\n' "$section" | head -n 1)"
if ! printf '%s' "$section_heading" | grep -iqE 'failure|what we|phase 4|manifest|catalog'; then
    printf 'check-chapter-05-failure-note: FAIL — section heading %q does not contain a failure-concept anchor\n' "$section_heading" >&2
    exit 1
fi
...
if [ ${#errors[@]} -gt 0 ]; then
    printf 'check-chapter-05-failure-note: %d element(s) missing:\n' "${#errors[@]}" >&2
    for e in "${errors[@]}"; do
        printf '  - %s\n' "$e" >&2
    done
    exit 1
...
printf 'check-chapter-05-failure-note: PASS — three explicit bold markers present + agent-synthesis tag on E3\n'
exit 0
