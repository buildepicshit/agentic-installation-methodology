...
Purpose: make every Copilot and Claude agent put documentation in the
...
| Path | Use for | Notes |
|---|---|---|
| `.agents/specs/` | Agent task specs, audit proposals, migration proposals, execution specs | Default location for work-control specs. These are executable contracts for agents. |
| `.agents/specs/SPEC.template.md` | Shared spec template | Copy/update for non-trivial tasks. |
| `.agents/skills/` | Copilot/shared skill procedures | Canonical shared skills. Claude copies mirror these under `.claude/skills/`. |
...
| `.agents/GREEN_ROOM_EVALUATION.md` | Post-handoff product evaluation and second-model verification protocol | Root-owned policy for green room audit packets. |
...
| `WORKFLOW.md` | Company-level engagement contract |
| `.agents/OPERATING_MODEL.md` | Canonical fleet operating model |
| `.agents/MODEL_ROUTING.md` | Agent/model selection and dispatch policy |
| `.agents/GREEN_ROOM_EVALUATION.md` | Green room product evaluation protocol |
...
Dispatch authority starts from each repo's own operating directive
(e.g. its `README.md`, build plan, or active v1 sequencing doc), not a
single fleet-wide path.
...
| Path | Use for |
|---|---|
| `.agents/specs/` | Agent audit and task-control specs; keep local unless approved |
