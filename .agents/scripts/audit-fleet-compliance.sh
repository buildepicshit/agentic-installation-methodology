...
#
# Usage:
#   bash agents/scripts/audit-fleet-compliance.sh                    audit all `applied` and `pending` directives
#   bash agents/scripts/audit-fleet-compliance.sh --validate <id>    validate directive format (no apply)
#   bash agents/scripts/audit-fleet-compliance.sh --feedback         report only AGENT_FEEDBACK entries
#
# Authority:
...
# excludes bes-fleet-policy (the policy repo audits itself separately).
# Strip it from the in-memory copy here:
INTERNAL_REPOS=("${INTERNAL_REPOS[@]/bes-fleet-policy/}")
...
audit_feedback() {
    ...
    for repo in "${ALL_REPOS[@]}" bes-fleet-policy; do
        local feedback_path
        if [ "$repo" = bes-fleet-policy ]; then
