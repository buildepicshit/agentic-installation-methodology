...
#
# Authority:
#   file://specs/2026-05-02-fleet-harness-enhancements/SPEC.md §8.2
#   file://agents/OPERATING_MODEL.md (entry-doc policy)
...
audit_internal_repo() {
    ...
    # the Fleet Rule Origination policy so agents working in the repo
    # know fleet rules originate in bes-fleet-policy.
    if [ ! -f "$repo_dir/AGENTS.md" ]; then
        ...
        if ! grep -qE 'Fleet Rule Origination' "$repo_dir/AGENTS.md"; then
            printf '%s/AGENTS.md:1: ADVISORY — does not reference "Fleet Rule Origination" (codified policy in OPERATING_MODEL); add a brief reference so agents in this repo know fleet rules originate in bes-fleet-policy\n' "$repo_name" >&2
    ...
    # WORKFLOW.md ADVISORY for internal repos (per-repo engagement contract).
    # Recommended by OPERATING_MODEL §"WORKFLOW.md Contract" for active
    # product repos; bes-fleet-policy is policy infrastructure and may
    # legitimately lack one. Auditor reports absence as advisory, not blocking.
    if [ ! -f "$repo_dir/WORKFLOW.md" ]; then
        printf '%s/WORKFLOW.md:1: ADVISORY — missing WORKFLOW.md (per-repo engagement contract; recommended for active product repos per OPERATING_MODEL §"WORKFLOW.md Contract")\n' "$repo_name" >&2
    elif [ "$repo_name" != "bes-fleet-policy" ]; then
    ...
    if [ "$repo_failures" -eq 0 ]; then
        printf 'PASS  %-20s  internal  (AGENTS.md + WORKFLOW.md present, sections + Fleet Rule Origination present, agent-specific imports correct)\n' "$repo_name"
    else
...
audit_public_oss_repo() {
    ...
    # workpad. Legacy AGENT_INBOX / AGENT_FEEDBACK files and active
    # SESSION_JOURNAL files carry BES-internal handoff content and
    # MUST be gitignored at OSS repos per OPERATING_MODEL
    # "Public OSS posture" — fleet-sync.sh adds them to .gitignore
    # on each sync.
