...
|---|---|---|
| `orient` | Starting any task or entering a repo | Repo map, source-of-truth list, verification gates, risks |
| `idea-capture` | Running the IDEA conversation before a SPEC exists | `IDEA.md` with constraints, 2-3 cited approaches, a recommendation, and owner-blocking questions separated out; gated to `status: ready-for-spec` |
| `author-spec` | Authoring a typed SPEC (fastpath / task / contract / decision) from a captured IDEA, or directly where no IDEA is required; owns the structured-options-cited-to-docs grill pattern | Structured `IDEA.md` and/or typed `SPEC.md` populated from the matching template, with citation discipline |
| `spec-driven-development` | Planning non-trivial work | Approved executable `SPEC.md` |
| `review-spec` | Reviewing a draft spec | Blocking findings, open questions, approval recommendation |
| `execute-spec` | Implementing approved scope | Scoped diff and updated tests/docs |
| `verify-spec` | Proving a change works | Command output summary and residual risk |
| `review-diff` | Reviewing an existing diff or PR | Bug/risk findings with file references |
| `spec-evidence` | Capturing durable lessons | Spec evidence candidates with provenance |
...
| `decompose-approved-spec` | An approved SPEC is large enough to split into vertical HITL or AFK slices | Subordinate decomposition plan with owned files, dependencies, acceptance evidence, and two-stage review order |
...
| `evidence-cluster` | Code authoring + review-spec | Impact-aware test selection (transitive graph) + anti-over-mock (no mocks in prod files) + complexity-delta gate during review + dual-reader ambiguity probe at SPEC review |
...
- Read `.agents/DOCUMENTATION_GUIDE.md` during orientation when the repo has
  one.
- Use `review-spec` before executing a spec written by another agent.
- Use `spec-evidence` only to produce evidence candidates. The approved
  spec and delivery record remain the trusted write boundary.
- Use `decompose-approved-spec` or `/decompose-approved-spec` only
