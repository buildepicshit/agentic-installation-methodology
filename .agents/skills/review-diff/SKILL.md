---
name: review-diff
description: "Use for reviewing local diffs, branches, or PRs. Prioritises bugs, regressions, missing tests, unsafe assumptions, and broken repo contracts over surface polish. Do not use to author or review SPEC.md / IDEA.md artefacts — use review-spec for that BLOCKING gate."
...
`decompose-approved-spec`, review in this order:
...
**Scope the scan to where the change lands.** Assess the subsystems the diff
actually touches, plus their immediate callers — not the whole tree. Widen only
on a concrete signal from the diff (a moved boundary, a new dependency edge, a
changed interface). Adopted 2026-07-24 from upstream Pocock
`improve-codebase-architecture` (`45afd80`), owner-directed: a whole-tree scan
on every review produces findings unrelated to the change under review, which
is reviewer output the author must triage and then defer. Pre-existing drift
outside the blast radius is a separate SPEC, not a finding against this diff.
...
`agents/skills/review-spec/SKILL.md`, `agents/specs/SPEC.schema.md`
